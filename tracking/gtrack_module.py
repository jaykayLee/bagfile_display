...
def module_update(inst, point, var, num):
    ...
    if True :
        for item in inst.activeList[:]:
            if item in need_removal:
                inst.activeList.remove(item)
    else :
    ...
    if False :    
        print(f'ActiveList={[ x.data for x in inst.activeList ]}')
        print(f'removal={[ x.data for x in need_removal ]}')
        print(f'FreeList={[ x.data for x in inst.freeList ]}')
